...
class ArtistModel {
    ...
    public function fetchArtists($category) {
        if ($category === 'onExhibition') {
            $sql = "
                SELECT DISTINCT a.id, a.artist_name AS name,a.artist_image AS imgUrl, a.artist_category AS field
            ...
            $sql = "
                SELECT id, artist_name AS name, artist_image AS imgUrl, artist_category AS field
